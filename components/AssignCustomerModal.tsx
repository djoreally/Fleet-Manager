...
// A simple, self-contained form for adding a new customer from within the modal
...
export const AssignCustomerModal: React.FC<AssignCustomerModalProps> = ({ inspectionId, onClose, onAssign }) => {
  ...
  const handleAssignCustomer = async (customerId: string) => {
    setIsAssigning(true);
    try {
        await localDB.inspections.update(inspectionId, { customerId, updatedAt: Date.now() });
        onAssign(customerId);
    } catch(error) {
    ...
    >
      ...
      >
        <div className="p-4 border-b border-line">
          <h3 className="text-lg font-semibold text-text-primary">Assign Customer to Inspection</h3>
          <p className="text-sm text-text-secondary">Inspection saved! Now, optionally assign a customer.</p>
        </div>
        <div className="flex">
                        ...
                        {filteredCustomers.map(customer => (
                            <li key={customer.id}>
                                <button 
                                    ...
                                    className="w-full text-left p-3 flex items-center gap-4 rounded-md hover:bg-line/50 transition-colors"
                                >
